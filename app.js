...
function isOlympicOnsiteEmployee(employee) {
  const nameParts = employee?.name?.trim().toLowerCase().split(/\s+/) || [];
  return OLYMPIC_ONSITE_NAMES.some(name => nameParts.includes(name));
}

function sanitizeOlympicOnsiteAssignments() {
  let changed = false;
  Object.entries(state.assignments).forEach(([key, employeeId]) => {
    const rowId = key.slice(0, key.lastIndexOf('-'));
    const row = rowById(rowId);
    const employee = employeeById(employeeId);
    if (!row || !employee) return;
    const valid = isOlympicOnsiteEmployee(employee) === (row.id === 'olympic-pm');
    if (!valid) {
...
function canAssign(employee, row, day) {
  ...
  const onsiteEmployee = isOlympicOnsiteEmployee(employee);
  if (onsiteEmployee && row.id !== 'olympic-pm') return false;
  if (row.id === 'olympic-pm' && !onsiteEmployee) return false;
...
function autoGenerate() {
  sanitizeOlympicOnsiteAssignments();
...
function adminLeavesPanel() {
  ...
  return `<section class="az-page-heading"><div><div class="eyebrow">TIME OFF MANAGEMENT</div><h1>Leave requests</h1><p>Approve, cancel, or delete requests. Approved dates are automatically blocked from the roster.</p></div><div class="az-count-badge">${pending} pending</div></section><section class="panel az-leave-table"><div class="az-table-head"><span>Employee</span><span>Dates</span><span>Reason</span><span>Status</span><span>Action</span></div>${state.leaves.length ? state.leaves.map(leave => { const employee = employeeById(leave.employee_id); const approvalActions = leave.status === 'pending' ? `<button class="primary-btn small-btn" data-leave-action="approve" data-leave-id="${leave.id}">Approve</button><button class="secondary-btn small-btn" data-leave-action="disapprove" data-leave-id="${leave.id}">Disapprove</button>` : ''; const cancelAction = ['pending', 'approved'].includes(leave.status) ? `<button class="secondary-btn small-btn" data-leave-action="cancel" data-leave-id="${leave.id}">Cancel</button>` : ''; return `<div class="az-table-row"><div><strong>${escapeHtml(employee?.name || 'Unknown employee')}</strong><small>${escapeHtml(employee?.email || '')}</small></div><div>${escapeHtml(longDate(leave.start_date))} – ${escapeHtml(longDate(leave.end_date))}</div><div>${escapeHtml(leave.reason || '—')}</div><div><span class="az-status ${escapeHtml(leave.status)}">${escapeHtml(leave.status)}</span></div><div class="az-actions">${approvalActions}${cancelAction}<button class="secondary-btn small-btn danger-btn" data-leave-action="delete" data-leave-id="${leave.id}">Delete</button></div></div>`; }).join('') : '<div class="az-empty-state">No leave requests yet.</div>'}</section>`;
}
...
function bindAdmin() {
  ...
  document.querySelectorAll('[data-leave-action]').forEach(button => button.addEventListener('click', () => { const action = button.dataset.leaveAction; if (action === 'approve' || action === 'disapprove') reviewLeave(button.dataset.leaveId, action === 'approve'); if (action === 'cancel') cancelLeave(button.dataset.leaveId); if (action === 'delete') deleteLeave(button.dataset.leaveId); }));
...
async function loadAdminRoster() {
  ...
  if (sanitizeOlympicOnsiteAssignments()) await saveRoster(true);
}
...
async function cancelLeave(id) {
  ...
  const { error } = await supabaseClient.from('leave_requests').update({ status: 'cancelled', reviewed_by: state.session.user.id, reviewed_at: reviewedAt }).eq('id', id);
  if (error) { toast(error.message); return; }
  leave.status = 'cancelled'; leave.reviewed_by = state.session.user.id; leave.reviewed_at = reviewedAt;
  renderAdmin(); toast('Leave request cancelled. The dates are available for rostering again.');
}

async function deleteLeave(id) {
  const leave = state.leaves.find(item => item.id === id);
  if (!leave || !window.confirm('Permanently delete this leave request? This cannot be undone.')) return;
  const { error } = await supabaseClient.from('leave_requests').delete().eq('id', id);
  if (error) { toast(error.message); return; }
  state.leaves = state.leaves.filter(item => item.id !== id);
  renderAdmin(); toast('Leave request deleted.');
}
