...
function bindAdmin() {
  ...
  $('addEmployee')?.addEventListener('click', () => { state.editingEmployeeId = null; renderAdmin(); });
...
async function saveEmployee(event) {
  ...
  const employee = { ...(current || {}), name: $('employeeName').value.trim(), email: $('employeeEmail').value.trim(), department: $('employeeDepartment').value, performance_score: Number($('employeeScore').value), max_days: Number($('employeeMaxDays').value), available_days: available, allowed_properties: properties, active: true };
  const { error } = await supabaseClient.from('team_members').upsert(employee, { onConflict: 'id' });
  if (error) { $('employeeError').textContent = error.message; $('employeeError').hidden = false; return; }
