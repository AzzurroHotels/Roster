...
function shell(content, options = {}) {
  ...
  return `<div class="app-shell ${admin ? 'az-admin-mode' : ''}">
    ${admin ? `<aside class="sidebar"><div class="brand"><div class="brand-mark">A</div><div><strong>Azzurro</strong><span>Admin operations</span></div></div><div class="sidebar-label">Admin workspace</div><nav><button class="nav-item ${state.adminPanel === 'roster' ? 'active' : ''}" data-admin-panel="roster"><span class="nav-icon">â–¦</span><span>Roster</span></button><button class="nav-item ${state.adminPanel === 'leaves' ? 'active' : ''}" data-admin-panel="leaves"><span class="nav-icon">â—·</span><span>Leave requests</span></button><button class="nav-item ${state.adminPanel === 'team' ? 'active' : ''}" data-admin-panel="team"><span class="nav-icon">â—Ž</span><span>Team & performance</span></button></nav><div class="sidebar-note"><span class="status-dot"></span><div><strong>Admin access</strong><small>${escapeHtml(state.session?.user?.email || '')}</small></div></div></aside>` : ''}
...
function renderPublic() {
  ...
  const content = `<header class="az-public-header"><div class="az-public-brand"><div class="brand-mark">A</div><div><strong>Azzurro Hotels</strong><span>Team roster</span></div></div><div class="az-public-actions"><span class="az-live-dot"></span><span>Published schedule</span>${state.session ? `<button class="secondary-btn" id="publicSignOut">Sign out</button>` : `<button class="secondary-btn" id="publicLogin">Admin / team login</button>`}</div></header><section class="az-public-heading"><div><div class="eyebrow">PUBLISHED ROSTER</div><h1>Reception roster</h1><p>Read-only team view Â· ${escapeHtml(weekLabel())}</p></div><div class="az-public-week"><button class="az-week-arrow" data-public-week="prev" aria-label="Previous week">â€¹</button><strong>${escapeHtml(weekLabel())}</strong><button class="az-week-arrow" data-public-week="next" aria-label="Next week">â€º</button></div></section><div class="tab-bar panel az-public-tabs">${DEPARTMENTS.map(department => `<button class="tab-btn ${tab === department.id ? 'active' : ''}" data-public-tab="${department.id}">${escapeHtml(department.label)}</button>`).join('')}</div>${publicRosterGrid(tab, data)}<section class="az-leave-panel panel"><div><div class="eyebrow">TIME OFF</div><h2>Need to file leave?</h2><p>Enter your name and requested dates. The request will be sent to admin for approval.</p></div><button class="primary-btn" id="openLeaveForm">File leave request</button></section>${state.leaveOpen ? leaveForm() : ''}`;
...
function adminRosterPanel() {
  ...
  return `<section class="control-strip panel"><div class="control-main"><div class="section-kicker">Plan a week</div><label for="adminWeek">Roster week (Mondayâ€“Sunday)</label><input id="adminWeek" type="week" value="${weekInputValue(state.weekStart)}" /></div><div class="rule-control"><label for="maxDays">Max days per person</label><input id="maxDays" type="number" min="1" max="7" value="5" /></div><div class="template-note"><span class="template-icon">â–¤</span><div><strong>Auto-generate</strong><small>Reception + Cleaners</small><small>Back office remains manual</small></div></div><button class="generate-btn" id="generateBtn"><span>âœ¦</span> Auto-generate roster</button></section><section class="tab-bar panel">${DEPARTMENTS.map(department => `<button class="tab-btn ${state.activeTab === department.id ? 'active' : ''}" data-admin-tab="${department.id}">${escapeHtml(department.label)}</button>`).join('')}</section><section class="roster-panel panel"><div class="panel-heading"><div><div class="section-kicker">Generated schedule</div><h2>${escapeHtml(template.label)} Â· ${escapeHtml(weekLabel())}</h2></div><div class="legend"><span><i class="legend-dot high"></i>High performance</span><span><i class="legend-dot standard"></i>Standard</span><span><i class="legend-dot empty"></i>Needs cover</span></div></div><div class="roster-scroll">${adminRosterGrid(state.activeTab)}</div><div class="roster-foot"><span>Approved leave dates are blocked from auto-generation and manual selection.</span><button class="secondary-btn" id="saveRoster">Save & publish roster</button></div></section>`;
}
...
function adminRosterGrid(tab) {
  ...
  const daysOff = DAYS.map((_, day) => state.employees.filter(employee => !workingDays(employee.id).has(day) && employee.department === tab).map(employee => employee.name).join(', ') || 'â€”');
...
function selectOptions(selected, row, day) {
  ...
  return `<option value="">Needs cover</option>${options.map(employee => `<option value="${escapeHtml(employee.id)}" ${selected === employee.id ? 'selected' : ''}>${escapeHtml(employee.name)} Â· ${Number(employee.performance_score || 0).toFixed(1)}</option>`).join('')}`;
}
